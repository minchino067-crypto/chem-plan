...
金沢大学 理系一括の6ヶ月計画（2026/8/31 – 2027/2/28、全26週）を回すためのWebアプリ。
化学120分・数学60分・英語60分の3科目。数英は共通テスト（第20週）で終わり、以降は化学のみ。
目標は総合800/1000（共テ数英8割＋二次化学8割）。
...
| 画面 | 役割 |
|---|---|
| 今日 | 起点。今日の小項目、期限の来た解き直し、やり残し |
| 今週 | 日ごと／まとめ（教材別）／見開き（7日を列に）の3つ |
| 全体 | 26週の一覧、または工程表（分野の帯と今日の位置） |
| 誤答 | 3分類の記録と偏り。記録は右下の「＋誤答」からどこでも |
| ルール・設定 | 右上の ⋯ から |
...
|---|---|
| `index.html` | 骨組み |
| `app.css` | 見た目。墨と和紙の階調。色は分野の識別にだけ使う |
| `app.js` | **26週×3科目の計画データ**と画面の全部。化学は `WEEKS`、数英は `SUBJ`（3科目PDFから取り込み）。教材の分類は `SRC` |
| `sync.js` | ログインとクラウド保存（Firebase） |
| `firebase-config.js` | **保存先と、開けるメールアドレス**。空だとアプリは開かない |
...
1日のタスクは `＋` で割って小項目にする（「基礎問 ＋ Ankiカード作成」は2件）。
`／` や `・` は「酸・塩基・塩」のように語の一部なので割らない。
小項目が全部終わると、その日が自動で完了になる。

「やらない」と決めた小項目はやり残しに出てこない。決めずに放置した分だけが
今日の画面に上がってくる。
